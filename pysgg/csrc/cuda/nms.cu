...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
    ...
    // 使用 at::cuda::empty 代替 THCudaMalloc
    at::Tensor mask_dev_t = at::empty({boxes_num * col_blocks}, 
                                      at::device(boxes.device()).dtype(at::kLong));
    unsigned long long* mask_dev = (unsigned long long*)mask_dev_t.data_ptr<int64_t>();
    ...
    cudaMemcpy(mask_host.data(), mask_dev, 
               sizeof(unsigned long long) * boxes_num * col_blocks, cudaMemcpyDeviceToHost);
    ...
    // 不需要显式 THCudaFree，因为 mask_dev 由 at::empty 创建，自动管理内存
