    ...
    const int part_size, const int sample_per_part, const float trans_std) 
{
  ...
  TORCH_CHECK(num_bbox == out.size(0), 
              "Output shape and bbox number must match: (", out.size(0), " vs ", num_bbox, ").");
...
{
  TORCH_CHECK(out_grad.is_contiguous(), "out_grad tensor must be contiguous");
  TORCH_CHECK(input.is_contiguous(), "input tensor must be contiguous");
  ...
  TORCH_CHECK(num_bbox == out_grad.size(0), 
              "Output shape and bbox number must match: (", out_grad.size(0), " vs ", num_bbox, ").");
